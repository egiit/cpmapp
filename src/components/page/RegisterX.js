...
const Register = () => {
  ...
  const [urlSave, seturlSave] = useState('http://localhost:3001/user');
  ...
  const getDepts = async () => {
    const respons = await axios.get('http://localhost:3001/dept');
    setDepts(respons.data);
  };

  const getMenus = async () => {
    const repsons = await axios.get('http://localhost:3001/menu');
    setMenus(repsons.data);
  };

  const getMenuAccess = async (id) => {
    const respons = await axios.get(`http://localhost:3001/useraccess/${id}`);
  ...
  const resetForm = () => {
    ...
    setbtnSubmit(true);
    setValidated(false);
    seturlSave('http://localhost:3001/user');
  };
  ...
  const saveUser = async (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        ...
        if (!btnSubmit) {
          await axiosJWT.patch(urlSave, dataUser);
          flash('User Update', 5000, 'success');
        } else {
          await axiosJWT.post(urlSave, dataUser);
  ...
  const findUser = async (id) => {
    setbtnSubmit(false);
    seturlSave(`http://localhost:3001/user/${id}`);
    const responseData = await axios.get(`http://localhost:3001/user/${id}`);
